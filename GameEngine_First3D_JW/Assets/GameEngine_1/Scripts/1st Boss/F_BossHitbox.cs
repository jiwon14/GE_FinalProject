...
/// <summary>
/// 보스의 공격 히트박스에 부착되어 플레이어와의 충돌을 감지하고 데미지를 처리하는 스크립트.
/// </summary>
[RequireComponent(typeof(Collider2D))]
public class F_BossHitbox : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        {
            Debug.LogError("F_BossHitbox가 부모에서 F_BossController를 찾을 수 없습니다!");
        }
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            Debug.Log($"보스 공격 적중: {other.name}");
            // 충돌한 대상을 기록하여 중복 데미지를 방지합니다.
            hitColliders.Add(other);

            // 플레이어 컨트롤러를 찾아 HandleAttack 함수를 호출합니다.
            other.GetComponent<F_PlayerController>()?.HandleAttack(damage, bossController.gameObject);
        }
    }
}